					  ...
					  struct mm_struct **mm_out)
{
	...
	*mm_out = NULL;
	...
	if (!bp_ticks_match(task, start_ticks)) {
		pr_info("bp: ticks mismatch tgid=%d user=%llu task_start_ns=%llu hz=%d\n",
			tgid, (unsigned long long)start_ticks,
			(unsigned long long)task->start_time, HZ);
...
static int bp_begin_session(const struct bp_session_v1 *u)
{
	...
	if (!g_bp_active) {
		if (u->expected_mm_cookie != 0) {
			pr_info("bp: session begin expected-cookie on fresh session tgid=%d exp=0x%llx\n", u->tgid, (unsigned long long)u->expected_mm_cookie);
			return -EINVAL;
		}
	} else {
...
static int bp_ptebp_arm_locked(const struct bp_pte_arm_v1 *u)
{
	...
	if (!u->pc || u->pc >= TASK_SIZE) {
		pr_info("bp: pte arm pc rejected pc=0x%llx\n", (unsigned long long)u->pc);
		return -EINVAL;
	}
...
static int bp_ptebp_mem_abort_adapter(struct pt_regs *hook_regs)
{
	...
	if (!hook_regs || !hook_regs->regs[2])
		return 0;
	local_irq_save(flags);
	asm volatile(".inst 0xd500409f" ::: "memory"); /* msr pan, #0 */
	ret = ptebp_handle_exec_fault(hook_regs);
	asm volatile(".inst 0xd500419f" ::: "memory"); /* msr pan, #1 */	local_irq_restore(flags);
	return ret;
}
...
static int bp_resolve_symbols(void)
{
	...
	/* EL0 sync-vector handler name differs across kernels:
	 * 5.15+ el0t_64_sync_handler; 5.10 do_mem_abort (vector labels are
	 * local symbols there) */
	sync_h = sync15;
	if (!sync_h)
		sync_h = generic_kallsyms_lookup_name("do_mem_abort");
	if (!sync_h)
		return -ENOENT;
