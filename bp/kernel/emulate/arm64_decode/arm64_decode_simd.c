...
static int arm64_normalize_simd_registers(struct arm64_decoded_insn *decoded)
{
    ...
    {
    case ARM64_SIMD_FORM_SCALAR_FP_IMMEDIATE:
    case ARM64_SIMD_FORM_VECTOR_IMMEDIATE:
        break;
    case ARM64_SIMD_FORM_SCALAR_FP_UNARY:
    case ARM64_SIMD_FORM_FP_GPR_TRANSFER:
    case ARM64_SIMD_FORM_CONVERT:
    case ARM64_SIMD_FORM_VECTOR_COPY:
    case ARM64_SIMD_FORM_VECTOR_SHIFT:
    case ARM64_SIMD_FORM_VECTOR_FP_UNARY:
    case ARM64_SIMD_FORM_VECTOR_REVERSE:
    case ARM64_SIMD_FORM_SCALAR_COPY:
    case ARM64_SIMD_FORM_FP_COMPARE_ZERO:
    case ARM64_SIMD_FORM_FP_REDUCE:
    case ARM64_SIMD_FORM_VECTOR_INTEGER_REDUCE:
    case ARM64_SIMD_FORM_VECTOR_NARROW:
    case ARM64_SIMD_FORM_SCALAR_NARROW:
        use_rn = 1;
        break;
    case ARM64_SIMD_FORM_SCALAR_FP_BINARY:
    case ARM64_SIMD_FORM_SCALAR_SELECT:
    case ARM64_SIMD_FORM_VECTOR_EXTRACT:
    case ARM64_SIMD_FORM_VECTOR_PERMUTE:
    case ARM64_SIMD_FORM_VECTOR_LOGICAL:
    case ARM64_SIMD_FORM_VECTOR_INTEGER_3REG:
    case ARM64_SIMD_FORM_VECTOR_EXTENDED_3REG:
    case ARM64_SIMD_FORM_VECTOR_FP_3REG:
    case ARM64_SIMD_FORM_VECTOR_FP_WIDENING_3REG:
    case ARM64_SIMD_FORM_VECTOR_COMPLEX_3REG:
    case ARM64_SIMD_FORM_FP_BY_ELEMENT:
    case ARM64_SIMD_FORM_SCALAR_SIMD_3REG:
    case ARM64_SIMD_FORM_VECTOR_BY_ELEMENT:
    case ARM64_SIMD_FORM_SCALAR_BY_ELEMENT:
        use_rn = 1;
        use_rm = 1;
        break;
    case ARM64_SIMD_FORM_SCALAR_FP_TERNARY:
        use_rn = 1;
        use_rm = 1;
        use_ra = 1;
        break;
    case ARM64_SIMD_FORM_SCALAR_COMPARE:
        use_rd = 0;
        use_rn = 1;
        use_rm = decoded->operands.simd.compare_operand == ARM64_SIMD_COMPARE_REGISTER;
        break;
...
static enum arm64_decode_status arm64_decode_simd_impl(uint32_t raw, struct arm64_decoded_insn *decoded)
{
    ...
    if ((raw & 0x9F208400U) == 0x0E008400U)
    {
        ...
        case 0x3F:
            ...
            else if (size == 3)
            {
                ...
                decoded->operands.simd.element_width = 16;
                decoded->operands.simd.result_element_width = 32;
                if (q) decoded->operands.simd.source_elements = ARM64_SIMD_ELEMENTS_ODD;
            }
        ...
        if (fp16 || (raw & 0x9F200400U) == 0x0E200400U)
        {
            ...
            if (!fp16 && !(size & 1))
            {
                ...
                else if (u && opcode == 25)
                {
                    ...
                    decoded->operands.simd.source_half = ARM64_SIMD_HALF_HIGH;
                }
    ...
    if ((raw & 0xDF000400U) == 0x0F000000U || (raw & 0xDF000400U) == 0x4F000000U || (raw & 0xDF000400U) == 0x5F000000U)
    {
        ...
        if ((size & 2) && !(opcode & 3) && ((opcode >> 3) == u))
        {
            ...
            decoded->operand_width = q ? 128 : 64;
            decoded->rm = (raw >> 16) & 0xF;
            if (u) decoded->operands.simd.source_half = ARM64_SIMD_HALF_HIGH;
            return ARM64_DECODE_OK;
        }
    ...
    if ((raw & 0xFF20FC00U) == 0x1E202000U)
    {
        ...
        decoded->operands.simd.compare_operand = zero ? ARM64_SIMD_COMPARE_ZERO : ARM64_SIMD_COMPARE_REGISTER;
...

    {
        ...
        else
        {
            ...
            if (operation != ARM64_SIMD_OP_NONE)
            {
                ...
                decoded->operand_width = raw & (1U << 30) ? 128 : 64;
                if (raw & (1U << 30)) decoded->operands.simd.destination_half = ARM64_SIMD_HALF_HIGH;
            }
        }
...
enum arm64_decode_status arm64_decode_simd(uint32_t raw, struct arm64_decoded_insn *decoded)
{
    ...
    if (status == ARM64_DECODE_OK && !arm64_normalize_simd_registers(decoded)) return ARM64_DECODE_UNSUPPORTED;
    return status;
}