...
static bool pid_is_digits(const char *s)
{
	if (!s || !*s)
		return false;
	while (*s)
...
static int read_task_cmdline(struct task_struct *task, char *buf, size_t size)
{
	struct mm_struct *mm;
	...
	if (_process_memory_rw(op_r, task->tgid, arg_start, buf, len) != 0)
	{
		mmput(mm);
		return -EFAULT;
	}
...
static bool cmdline_matches(const char *cmd, const char *name, bool *exact)
{
	int nlen;
	...
	if (!cmd || !name || !*cmd || !*name)
		return false;
	nlen = strlen(name);
	if (strncmp(cmd, name, nlen) == 0 && (cmd[nlen] == '\0' || cmd[nlen] == ' '))
	{
		if (exact)
			*exact = true;
		return true;
	}
	return strstr(cmd, name) != NULL;
}

static bool comm_matches_package(const char *comm, const char *name)
{
	int clen;
	...
	/* Android task->comm is limited to TASK_COMM_LEN (16), so long package
	 * names are truncated, e.g. com.tencent.tmgp.sgame -> com.tencent.tmg.
	 */
	if (strncmp(name, comm, clen) == 0)
		return true;
	if (strstr(name, comm) != NULL)
		return true;
	return strstr(comm, name) != NULL;
}

static int kernel_find_process_by_name(struct process_select_info *info)
{
	struct task_struct *task;
	int exact_pid = -1, best_pid = -1;
	uint64_t exact_rss = 0, best_rss = 0;
	char cmd[PROC_NAME_LEN];

	if (!info || !info->name[0])
		return -EINVAL;
	info->selected_pid = -1;
	info->selected_rss_kb = 0;

	if (pid_is_digits(info->name))
	{
		int pid = simple_strtol(info->name, NULL, 10);
		...
		if (t)
		{
			...
			if (mm)
			{
				info->selected_rss_kb = (uint64_t)get_mm_rss(mm) << (PAGE_SHIFT - 10);
				mmput(mm);
			}
		}
		put_pid(p);
		return info->selected_pid > 0 ? 0 : -ESRCH;
	}
	...
	for_each_process(task)
	{
		...
		cmd_len = read_task_cmdline(task, cmd, sizeof(cmd));
		if (cmd_len > 0)
			matched = cmdline_matches(cmd, info->name, &exact);

		if (!matched && comm_matches_package(task->comm, info->name))
			matched = true;
		...
		if (mm)
		{
			rss = (uint64_t)get_mm_rss(mm) << (PAGE_SHIFT - 10);
			mmput(mm);
		}
	...
	if (exact_pid > 0)
	{
		info->selected_pid = exact_pid;
		info->selected_rss_kb = exact_rss;
		return 0;
	}
	if (best_pid > 0)
	{
		info->selected_pid = best_pid;
		info->selected_rss_kb = best_rss;
		return 0;
	}
	return -ESRCH;
}

static int set_process_hwbp_kernel_select(struct req_obj *r)
{
	int ret;
	if (!r)
		return -EINVAL;
	if (r->pid <= 0 && r->proc_info.name[0])
	{
		ret = kernel_find_process_by_name(&r->proc_info);
		if (ret)
			return ret;
		r->pid = r->proc_info.selected_pid;
	}
	return set_process_hwbp(r->pid, &r->bp_info);
}
		...
		if (ProcessExit)
		{
			...
			if (req->kernel)
			{
				...
				case op_brps_weps_info:
					...
					break;
				case op_find_process_by_name:
					req->status = kernel_find_process_by_name(&req->proc_info);
					if (req->status == 0)
						req->pid = req->proc_info.selected_pid;
					break;
				case op_set_process_hwbp:
					req->status = set_process_hwbp_kernel_select(req);
					break;
