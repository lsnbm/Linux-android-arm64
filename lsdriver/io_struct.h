...
{
    char name[PROC_NAME_LEN]; // 用户态只写包名/进程名，内核负责查找 PID
    int selected_pid;         // 内核选中的 PID
    uint64_t selected_rss_kb; // 内核统计的 RSS，单位 KB
};
...
enum sm_req_op
{
    ...
    op_find_process_by_name, // 内核按包名/进程名查找进程
    op_set_process_hwbp,     // 设置硬件断点，pid<=0 时可使用 proc_info.name 内核查找
...
struct req_obj
{
    ...
    int pid; // 当前派发指定的pid
    struct process_select_info proc_info; // 内核态进程选择信息
