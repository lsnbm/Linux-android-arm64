...
static int ptebp_emu_read_mem(void *opaque, uint64_t addr, int bytes, __uint128_t *out)
{
    ...
    {
        const struct ptebp_emu_patch *patch = &ctx->patches[patch_index];
        uint64_t start = max(addr, patch->hook_addr);
        uint64_t patch_end = patch->hook_addr + sizeof(patch->orig_insn);
        uint64_t stop = min(end, patch_end);
        size_t data_offset;
        size_t insn_offset;
        size_t copy_size;
...
static int ptebp_handle_fault(struct pt_regs *hook_regs)
{
    ...
    mem_ctx.page_paddr = PFN_PHYS(pte_pfn(owner->orig_pte));
