...
#define HWBP_STACK_MAX 256

static inline void hwbp_capture_stack(struct hwbp_record *rec, unsigned long sp)
{
    uint64_t value;
    uint32_t i;

    rec->stack_count = 0;
    rec->stack_reserved = 0;

    if (!sp)
        return;

    for (i = 0; i < HWBP_STACK_MAX; i++)
    {
        if (copy_from_user(&value, (const void __user *)(sp + i * sizeof(uint64_t)), sizeof(value)) != 0)
            break;
        rec->stack[i] = value;
        rec->stack_count++;
    }
...
static inline void sample_hbp_handler(struct pt_regs *regs, struct breakpoint_config *self)
{
    ...
    if (rec)
    {
        ...
        hwbp_capture_stack(rec, regs->sp);
